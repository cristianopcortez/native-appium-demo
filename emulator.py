...
class Emulator(Device):
    ...
    def change_permission(self) -> None:
        ...
        if os.path.exists(kvm_path):
            ...
            self.logger.warning("KVM not found - running with software rendering (swiftshader_indirect)")

    def deploy(self):
        self.logger.info(f"Deploying the {self.device_type}")

        kvm_available = os.path.exists("/dev/kvm")
        accel_flag = "-accel on" if kvm_available else "-accel off"
        basic_cmd = "emulator @{n}".format(n=self.name)
        basic_args = f"-gpu swiftshader_indirect {accel_flag} -writable-system -verbose"
